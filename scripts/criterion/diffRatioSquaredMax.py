...
@dataclass
class MultiRefDiffRatioSquaredMaxLossConfig(FairseqDataclass):
    sentence_avg: bool = II("optimization.sentence_avg")

@register_criterion("multi_ref_diff_ratio_squared_max_loss", dataclass=MultiRefDiffRatioSquaredMaxLossConfig)
class MultiRefDiffRatioSquaredMaxLoss(FairseqCriterion):
    ...
    def my_loss_loop_01(
        ...
        for (main_LoD, main_target, lprobs, sub_LoDs, sub_targets) in \
            ...
            sum_diff = torch.tensor(0.0, device=lprobs.device)
            all_diff = torch.tensor(0.0, device=lprobs.device)
            ...
            for (sub_LoD, sub_target) in zip(sub_LoDs, sub_targets):
                ...
                all_diff += ( torch.square(main_LoD - sub_LoD) * torch.abs(L_target - L_sub) )

                # L_subの方がL_targetよりも大きい場合のみ加算
                if (L_target - L_sub) > 0:
                    sum_diff += ( torch.square(main_LoD - sub_LoD) * (L_target - L_sub))

            ###
            # L_target * ( 1 + alpha * ( 1 / sum(d_i^2) ) * R)
            # R = sum(d_i^2 * (L_target - L_sub_i)) (if L_target - L_sub_i > 0)
            ###
            tmp_loss = L_target * ( 1 + alpha * ( sum_diff / all_diff ) )
