...
@dataclass
class MultiRefDiffValLossConfig(FairseqDataclass):
    sentence_avg: bool = II("optimization.sentence_avg")

@register_criterion("multi_ref_diff_val_loss", dataclass=MultiRefDiffValLossConfig)
class MultiRefDiffValLoss(FairseqCriterion):
    ...
    def my_loss_loop_01(
        ...
        for (main_LoD, main_target, lprobs, sub_LoDs, sub_targets) in \
            ...
            for (sub_LoD, sub_target) in zip(sub_LoDs, sub_targets):
                ...
                ###
                tmp_diff = torch.abs((torch.abs(main_LoD - sub_LoD) * L_main) - L_sub)
