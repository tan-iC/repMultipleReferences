...
@dataclass
class MultiRefDiffMaxLossConfig(FairseqDataclass):
    sentence_avg: bool = II("optimization.sentence_avg")

@register_criterion("multi_ref_diff_max_loss", dataclass=MultiRefDiffMaxLossConfig)
class MultiRefDiffMaxLoss(FairseqCriterion):
    ...
    def my_loss_loop_01(
        ...
        for (main_LoD, main_target, lprobs, sub_LoDs, sub_targets) in \
            ...
            for (sub_LoD, sub_target) in zip(sub_LoDs, sub_targets):
                ...
                ###
                diffMax = torch.max(torch.Tensor([(L_main - L_sub).sum(), 0]))
                diffMax = diffMax.to(lprobs.device)
                tmp_diff = (torch.abs(main_LoD - sub_LoD) * diffMax)
