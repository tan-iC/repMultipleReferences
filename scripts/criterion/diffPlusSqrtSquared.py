...
@dataclass
class MultiRefDiffPlusSqrtSquaredLossConfig(FairseqDataclass):
    sentence_avg: bool = II("optimization.sentence_avg")

@register_criterion("multi_ref_diff_plus_sqrt_squared_loss", dataclass=MultiRefDiffPlusSqrtSquaredLossConfig)
class MultiRefDiffPlusSqrtSquaredLoss(FairseqCriterion):
    ...
    def my_loss_loop_01(
        ...
        for (main_LoD, main_target, lprobs, sub_LoDs, sub_targets) in \
            ...
            for (sub_LoD, sub_target) in zip(sub_LoDs, sub_targets):
                ...
                ###
                tmp_diff = torch.square( (torch.sqrt(torch.square(main_LoD - sub_LoD) + 1) * L_main) - L_sub)
