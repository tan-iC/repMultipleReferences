...
@dataclass
class MultiRefLogRatioLossConfig(FairseqDataclass):
    sentence_avg: bool = II("optimization.sentence_avg")

@register_criterion("multi_ref_log_ratio_loss", dataclass=MultiRefLogRatioLossConfig)
class MultiRefLogRatioLoss(FairseqCriterion):
    ...
    def my_loss_loop_01(
        ...
        for (main_LoD, main_target, lprobs, sub_LoDs, sub_targets) in \
            ...
            for (sub_LoD, sub_target) in zip(sub_LoDs, sub_targets):
                ...
                # 比の計算
                ###
                # d_i * log(L_main / L_sub)
                ###
                tmp_rate = torch.abs(main_LoD - sub_LoD) * torch.log(L_main / L_sub)
            ...
            # 記録
            # with open('/mntdir/tani/work/myFairseqScripts/multipleReferences/data/log/loss.txt', mode="a") as f:
            #     f.write(f'L_main: {L_main}, L_sub: {L_sub}, sum_rate: {sum_rate}, tmp_loss: {tmp_loss}, loss: {loss}\n')
