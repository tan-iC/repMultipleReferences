...
@dataclass
class MultiRefDiffCompSquaredMaxLossConfig(FairseqDataclass):
    sentence_avg: bool = II("optimization.sentence_avg")

@register_criterion("multi_ref_diff_comp_squared_max_loss", dataclass=MultiRefDiffCompSquaredMaxLossConfig)
class MultiRefDiffCompSquaredMaxLoss(FairseqCriterion):
    ...
    def my_loss_loop_01(
        ...
        for (main_LoD, main_target, lprobs, sub_LoDs, sub_targets) in \
            ...
            for (sub_LoD, sub_target) in zip(sub_LoDs, sub_targets):
                ...
                all_diff += ( torch.square(main_LoD - sub_LoD) )
                tmp_diff = torch.tensor(0.0, device=lprobs.device)

                # L_subの方がL_targetよりも大きい場合のみ加算
                if (L_target - L_sub) > 0:
                    tmp_diff += ( torch.square(main_LoD - sub_LoD) )
            ...
            # R = sum(d_i^2) (if L_target - L_sub_i > 0)
            ###
            tmp_loss = L_target * ( 1 + alpha * ( 1 / all_diff ) * sum_diff )
